...
public class ExceptionTranslator implements ProblemHandling, SecurityAdviceTrait {
  ...
  private static HttpHeaders createFailureAlert(String applicationName, boolean enableTranslation,
  ...
  @Override
  public ProblemBuilder prepare(Throwable throwable, StatusType status,
      URI type) {
    ...
    if (activeProfiles.contains("prod")) {
      if (throwable instanceof HttpMessageConversionException) {
        return Problem
            ...
            .withCause(
                Optional.ofNullable(throwable.getCause()).filter(cause -> this
    ...
    return Problem
        ...
        .withCause(
            Optional.ofNullable(throwable.getCause()).filter(cause -> this.isCausalChainsEnabled())
                .map(this::toProblem).orElse(null)
        );
  }
